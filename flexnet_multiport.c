...
void usage( char *cmd) {
    ...
    fprintf( stderr, "       %s [-v] [-D] -d <device> -s <speed> disk_image (legacy)\n", cmd);
    fprintf( stderr, "Options:\n");
    fprintf( stderr, " -c <config> : YAML configuration file (multi-port mode)\n");
    fprintf( stderr, " -d <device> : serial line to use (single port legacy mode)\n");
    fprintf( stderr, " -s <speed> : baudrate to use (single port legacy mode)\n");
    ...
    fprintf( stderr, "  - device: /dev/ttyS0\n");
    fprintf( stderr, "    speed: 19200\n");
    fprintf( stderr, "    disk: disk1.dsk\n");
    fprintf( stderr, "  - device: /dev/ttyUSB0\n");
    fprintf( stderr, "    speed: 9600\n");
    fprintf( stderr, "    disk: disk2.dsk\n");
}
...
static char *pid_file = "/var/run/flexnet.pid";
char param[128];
...
void signal_handler(int sig) {
    if (sig == SIGTERM || sig == SIGINT) {
        ...
        for (int i = 0; i < num_ports; i++) {
            ...
            if (ports[i].fd_disk > 0) {
                close(ports[i].fd_disk);
            }
        }
        closelog();
        exit(0);
...
/**
 * Parse YAML configuration file to load port configurations
 * 
...
int load_config(const char *config_path) {
    ...
    if (!yaml_parser_load(&parser, &document)) {
        fprintf(stderr, "Error: Failed to load YAML document: %s\n", parser.problem);
    ...
    for (yaml_node_pair_t *pair = root->data.mapping.pairs.start;
        ...
        if (key->type == YAML_SCALAR_NODE && 
            strcmp((char*)key->data.scalar.value, "ports") == 0) {
            ports_node = value;
            break;
        }
    }
    ...
    if (!ports_node || ports_node->type != YAML_SEQUENCE_NODE) {
        fprintf(stderr, "Error: 'ports' must be a sequence in YAML config\n");
        yaml_document_delete(&document);
        yaml_parser_delete(&parser);
        fclose(fh);
        return -1;
    ...
    // Parse each port configuration
    num_ports = 0;
    for (yaml_node_item_t *item = ports_node->data.sequence.items.start;
         item < ports_node->data.sequence.items.top && num_ports < MAX_PORTS;
         item++) {
        yaml_node_t *port = yaml_document_get_node(&document, *item);
        
        if (port->type != YAML_MAPPING_NODE) {
            fprintf(stderr, "Error: Each port must be a mapping\n");
            continue;
        }
        
        // Initialize port config
        memset(&ports[num_ports], 0, sizeof(port_config_t));
        ports[num_ports].fd_disk = -1;
        strcpy(ports[num_ports].curdir, ".");
        
        // Parse port mapping
        for (yaml_node_pair_t *pair = port->data.mapping.pairs.start;
             pair < port->data.mapping.pairs.top; pair++) {
            yaml_node_t *key = yaml_document_get_node(&document, pair->key);
            yaml_node_t *value = yaml_document_get_node(&document, pair->value);
            
            if (key->type == YAML_SCALAR_NODE && value->type == YAML_SCALAR_NODE) {
                char *key_str = (char*)key->data.scalar.value;
                char *value_str = (char*)value->data.scalar.value;
                
                if (strcmp(key_str, "device") == 0) {
                    strncpy(ports[num_ports].device, value_str, sizeof(ports[num_ports].device) - 1);
                } else if (strcmp(key_str, "speed") == 0) {
                    ports[num_ports].speed = atoi(value_str);
                } else if (strcmp(key_str, "disk") == 0) {
                    strncpy(ports[num_ports].disk_image, value_str, sizeof(ports[num_ports].disk_image) - 1);
                }
            }
        }
        
        // Validate required fields
        if (ports[num_ports].device[0] == 0 || ports[num_ports].speed == 0 || 
            ports[num_ports].disk_image[0] == 0) {
            fprintf(stderr, "Error: Port %d missing required fields (device, speed, disk)\n", num_ports);
            continue;
        }
        
        if (verbose) {
            log_message(LOG_INFO, "Loaded port %d: %s at %d baud, disk: %s", 
                       num_ports, ports[num_ports].device, ports[num_ports].speed,
                       ports[num_ports].disk_image);
        }
    ...
    if (num_ports == 0) {
        fprintf(stderr, "Error: No valid ports configured\n");
        return -1;
    }
 ...
 * 
 * @param port Port configuration to use for disk geometry
 ...
 */
int ts2blk(port_config_t *port, uint8_t ntrk, uint8_t nsec)
{
    // Validate track and sector numbers
    if (ntrk > port->nbtrk || nsec > port->nbsec || (nsec == 0 && ntrk != 0)) {
        return( -1);
    }
    ...
    if (ntrk == 0) {        // Track 0 has special handling
        ...
        return port->track0l + (ntrk - 1) * port->nbsec + nsec - 1;
    }
}
