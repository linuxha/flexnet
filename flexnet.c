...
static int verbose = 0;     // Debug output flag (set with -v option)\nstatic int daemon_mode = 0; // Daemon mode flag (set with -D option)\nstatic char *pid_file = \"/var/run/flexnet.pid\"; // PID file path
...
int main(int argc, char **argv)
{
    ...
    // Read parameters
    while ((opt = getopt( argc, argv, "d:s:vhDV")) != -1) {
        switch (opt) {
        case 'h':
            usage( *argv);
            exit( 0);
